...
import Logo2 from "../assets2/logo2.png";
import Logo3 from "../assets2/logo3.png";
...
import { getCategoriesData } from "../services/operation/CatalogAndCategoriesDetails";
...
const Navbar = () => {
  const {token} = useSelector((state)=> state.auth);
    ...
    // console.log("length",subLinks.categories.length);
  ...
  return (
    ...
    <div className='flex w-11/12 max-w-maxContent items-center justify-between'>
      {/* Image */}
    <Link to="/">
      <img src={Logo2} width={160} height={42} loading='lazy'/>
    </Link>
    ...
    <nav>
               ...
               <li key={index}>
                  {
                      link.title === "Catalog" ? (
                          <div className='relative flex items-center gap-2 group'>
                              <p className=" hover:text-yellow-25 hover:underline underline-offset">{link.title}</p>
                              <IoIosArrowDown/>
                          ...
                          <Link to={link?.path}>
                            <p className={`${link?.path === "/" ? "text-yellow-25" : "text-richblack-25"} hover:text-yellow-25 hover:underline underline-offset`}>
      {link.title}
    </p>
      ...
      <div className='hidden md:flex gap-x-4 items-center'>
          {   
              user && user?.accountType != "Instructor" && (
                  <Link to="/dashboard/cart" className='relative pr-2'>
                      <AiOutlineShoppingCart className='text-2xl text-richblack-100 hover:text-pink-200 ' />
                      {
                          totalItems > 0 && (
          ...
          }
          {
              token === null && (
                  <Link to="/login">
                      <button className='  px-[12px] py-[8px] text-yellow-50 rounded-md hover:bg-richblack-800 hover:scale-105'>
                          Log In
                      </button>
                  </Link>
              )
          }
          {
              token === null && (
                  <Link to="/signup">
                      <button  className='border border-richblack-700 bg-yellow-100 px-[12px] py-[8px] text-pure-greys-5  rounded-md hover:scale-105'>
                          Sign Up
                      </button>
                  </Link>
              )
